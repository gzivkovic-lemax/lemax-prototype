      ...
      <section class="lmx-card lmx-grid-card">
        <div class="lmx-grid-scroll">
          <table class="lmx-data-grid accommodation-grid">
            ...
            <tbody>
              <tr *ngFor="let row of filteredAccommodations()" (dblclick)="openEditor(row)">
                <td>{{ row.code }}</td>
                <td>{{ row.name }}</td>
                <td>{{ row.country }}</td>
                <td>{{ row.region }}</td>
...
export class AccommodationPageComponent {
  ...
  protected readonly filteredAccommodations = computed(() => {
    ...
    return this.accommodations().filter((row) =>
  ...
  protected openNew(): void {
    this.windowManager.open('prototype-accommodation', 'new', 'New accommodation', 'edit');
  }
}
