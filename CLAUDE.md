...
- Action (primary CTAs — Group actions / OK / New customer): `--lemax-action` `#FF4A73`, `--lemax-action-dark` `#e63566`.
...
### Brand color rule (MANDATORY)

This rule is load-bearing for visual fidelity — apply it whenever you add or restyle a button.

- **Primary action buttons** (Save / OK / New customer / Group actions / "New …" CTAs and other write/commit gestures) — **Lemax pink `#FF4A73`** via `--lemax-action`. Use `.lmx-btn--action` for solid pink, `.lmx-btn--action-outline` for pink-on-white.
- **Filter, search, submit-filter, and chrome accent buttons** + the topbar / window header / active tab underline / link color — **Lemax blue `#00A6E5`** via `--lemax-blue`. Use `.lmx-btn--blue` for the filter card's submit button and similar blue affordances.
